...
module.exports = (env) ->
  ...
  class PushbulletActionProvider extends env.actions.ActionProvider
    ...
    parseAction: (input, context) =>
      ...
      defaultChannel = @config.channeltag
      ...
      setChannel = (m, tokens) =>
        ...
        deviceTokens = strToTokens ""
      setDevice = (m, tokens) =>
        channelTokens = strToTokens ""
        deviceTokens = tokens
      ...
      next = m.match(' email:').matchStringWithVars(setDevice)
      if next.hadMatch() then m = next
      ...
      if m.hadMatch()
        ...
        return {
          ...
          actionHandler: new PushbulletActionHandler(
            @framework, titleTokens, messageTokens, typeTokens, deviceTokens, channelTokens
          )
        }
  ...
  class PushbulletActionHandler extends env.actions.ActionHandler
    ...
    constructor: (@framework, @titleTokens, @messageTokens, @typeTokens, @deviceTokens, @channelTokens) ->
    ...
    executeAction: (simulate, context) ->
      ...
      ]).then( ([title, message, type, device, channel]) =>
        # If a channel is set, use it as target device
        if channel != ""
          device = { channel_tag: channel }
